...
<script type="text/javascript">
  ...
  Morris.Line({
    ...
    data: [
           <% List ls=(List)request.getAttribute("lineData");
           if(ls!=null){
        	   for(int x=0;ls.size()>x;x++){
        		   Map m=(Map)ls.get(x);
           %>
            {y: '<%=m.get("myear")%>', a: <%=m.get("projet")%>, b: <%=m.get("agent")%>, c: <%=m.get("complaint")%>},
      ...
      Morris.Bar({
         element: 'bar-example',
         data: [
                <%  ls=(List)request.getAttribute("barData");
                if(ls!=null){
             	   for(int x=0;ls.size()>x;x++){
             		   Map m=(Map)ls.get(x);
                %>
    	...
    	// stacked:true,
         ...
         xkey: 'y',
         ykeys: ['a','b','c'],
         labels: ['Total Project Registration Fee', 'Total Project Registration Fee','Total Project Registration Fee']
      });
